...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        protected virtual async void CrawlSite(CrawlResult crawlResult)
        {
            ...
            while (!CrawlComplete)
            {
                ...
                if (CrawlContext.Scheduler.Count > 0)
                {
                    ...
                    // TODO async 
                    Task task = new Task(() => ProcessPage(CrawlContext.Scheduler.GetNext(), crawlResult));
                    tasks.Add(task);
                    task.Start();

                    //ThreadManager.DoWork(() => ProcessPage(CrawlContext.Scheduler.GetNext(), crawlResult));

                }
                else if (tasks.All(x => x.Status == TaskStatus.RanToCompletion))
                {
                    CrawlComplete = true;
                }
                else
                ...
                //else if (taskFactory.Scheduler.)//(!ThreadManager.HasRunningThreads())
                //{
                //    CrawlComplete = true;
                //}
                //else
        ...
        protected virtual void ProcessPage(PageToCrawl pageToCrawl, CrawlResult crawlResult)
        {
            try
            {
                ...
                if (IsRedirect(crawledPage) && !CrawlContext.CrawlConfiguration.IsHttpRequestAutoRedirectsEnabled)
        ...
        protected virtual CrawledPage CrawlThePage(PageToCrawl pageToCrawl)
        {
            ...
            CrawledPage crawledPage = PageRequester.MakeRequest(pageToCrawl.Uri, ShouldDownloadPageContent);
